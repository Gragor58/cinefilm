<?php
    ...
    try{
        $db = new PDO('mysql:host=localhost;dbname=dbFilms;charset=utf8', 'root', 'online@2017');
        ...
        $answer1 = $db->prepare('SELECT * FROM films WHERE titre LIKE :searchStart
         GROUP BY titre ORDER BY `films`.`titre` ASC');

        $answer2 = $db->prepare('SELECT * FROM films WHERE titre LIKE :searchMid
         GROUP BY titre ORDER BY `films`.`titre` ASC');

        $answer3 = $db->prepare('SELECT * FROM films WHERE titre LIKE :searchEnd
         GROUP BY titre ORDER BY `films`.`titre` ASC');
